...
class Enemy extends GameObject {
    ...
    constructor(x, y, type, pattern, canvasWidth, difficultyConfig, level = 1) {
        ...
        // Level-based scaling: enemies get tougher each level
        // HP: +6% per level (1.0x at L1, ~2.74x at L30)
        // Speed: +2% per level (1.0x at L1, ~1.58x at L30) 
        const levelHpMult = 1 + (level - 1) * 0.06;
        const levelSpdMult = 1 + (level - 1) * 0.02;
...
class MultiBoss {
    constructor(x, y, bossId, canvasWidth, isMiniBoss = false, difficultyConfig = null, level = 1) {
        ...
        // Level-based scaling for bosses/mini-bosses
        // HP: +5% per level, Speed: +1.5% per level
        const levelHpMult = 1 + (level - 1) * 0.05;
        const levelSpdMult = 1 + (level - 1) * 0.015;
    ...
    render(ctx, assets) {
        ...
        ctx.font = this.isMiniBoss ? 'bold 9px monospace' : 'bold 10px monospace';
