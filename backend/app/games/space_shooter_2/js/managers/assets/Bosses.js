...
//  Aesthetic: elegant quantum-physics motifs — wave functions,
...
function _genBoss19Sprites(sprites) {
    ...
    function _quarkCore(spriteKey, fill, iris, accent, dark, expression) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const cp1a = a + Math.PI / 6, cp2a = a + Math.PI / 3;
    ...
    // Gluon turret — small swirling energy knot
    {
        ...
        g.addColorStop(0, '#ffffff'); g.addColorStop(0.3, '#ffdd44'); g.addColorStop(1, '#885500');
...
function _genBoss20Sprites(sprites) {
    ...
    // EM turret — crackling lightning node
    {
        ...
        g.addColorStop(0, '#ffffff'); g.addColorStop(0.3, '#44ddff'); g.addColorStop(1, '#0055aa');
        ctx.fillStyle = g; ctx.fill(); ctx.strokeStyle = '#111'; ctx.lineWidth = 2; ctx.stroke();
        // Lightning tendrils
...
function _genBoss21Sprites(sprites) {
    ...
    // Core (85x85 → 105x105) — Majestic chromodynamic face
    {
        ...
        for (const e of eyeData) {
            ...
            ig.addColorStop(0, '#ffffff'); ig.addColorStop(0.3, e.iris); ig.addColorStop(1, '#000');
    ...
    // Charge turret — color-charge orbs
    {
        ...
        cg.addColorStop(0, '#ffffff'); cg.addColorStop(0.2, accent);
...
function _genBoss22Sprites(sprites) {
    const color = '#ffd700', accent = '#ffee88', dark = '#886600';
    ...
    // Weakpoint — exposed Higgs field node
    {
        ...
        g.addColorStop(0, '#ffffff'); g.addColorStop(0.3, accent); g.addColorStop(0.7, color); g.addColorStop(1, dark);
...
function _genBoss23Sprites(sprites) {
    // Matter core (left) — warm, structured, ordered face
    {
        const color = '#4488ff', accent = '#88bbff', dark = '#223388';
    ...
    {
        const color = '#cc44ff', accent = '#dd88ff', dark = '#552288';
        const S = 75, cv = _mkCanvas(S, S), ctx = cv.getContext('2d'), cx = S / 2, cy = S / 2, r = 30;
    ...
    // Matter turret — orderly angular
    {
        ...
        outlineAndFill(ctx, '#4488ff', '#111', 2);
    ...
    // Antimatter turret — chaotic spiky
    {
        ...
        ctx.closePath();
        outlineAndFill(ctx, '#cc44ff', '#111', 2);
...
function _genBoss24Sprites(sprites) {
    const color = '#ffffff', accent = '#ddddff', dark = '#555577';
    // Core (100x100 → 120x120) — Majestic unified-field entity with 4-force mandala face
    {
        ...
        bg.addColorStop(0, '#ffffff'); bg.addColorStop(0.3, '#eeeeff');
    ...
    // EM turret — electric spark node
    {
        ...
        g.addColorStop(0, '#ffffff'); g.addColorStop(0.3, '#44ddff'); g.addColorStop(1, '#003366');
    ...
    // Strong turret — green flux tube node
    {
        const S = 44, cv = _mkCanvas(S, S), ctx = cv.getContext('2d'), cx = S / 2, cy = S / 2;
        const g = ctx.createRadialGradient(cx, cy, 0, cx, cy, 13);
        g.addColorStop(0, '#ffffff'); g.addColorStop(0.3, '#44ff88'); g.addColorStop(1, '#003311');
        ctx.fillStyle = g; ctx.beginPath(); ctx.arc(cx, cy, 13, 0, Math.PI * 2); ctx.fill();
        ctx.strokeStyle = '#111'; ctx.lineWidth = 2; ctx.stroke();
        // Triple gluon lines
        ctx.lineWidth = 1;
    ...
    // Inner orbit turret
    {
        const S = 36, cv = _mkCanvas(S, S), ctx = cv.getContext('2d'), cx = S / 2, cy = S / 2;
        const g = ctx.createRadialGradient(cx, cy, 0, cx, cy, 10);
        g.addColorStop(0, '#ffffff'); g.addColorStop(0.4, accent); g.addColorStop(1, dark);
        ctx.fillStyle = g; ctx.beginPath(); ctx.arc(cx, cy, 10, 0, Math.PI * 2); ctx.fill();
        ctx.strokeStyle = '#111'; ctx.lineWidth = 2; ctx.stroke();
        // Tiny face
        ctx.fillStyle = '#333';
    ...
    // Weakpoint — unified field crack
    {
        ...
        g.addColorStop(0, '#ffffff'); g.addColorStop(0.3, '#ffdd88'); g.addColorStop(0.7, '#ff8844'); g.addColorStop(1, '#442200');
        ctx.fillStyle = g; ctx.beginPath(); ctx.arc(cx, cy, 14, 0, Math.PI * 2); ctx.fill();
        ctx.strokeStyle = '#111'; ctx.lineWidth = 2; ctx.stroke();
        // Crack lines
