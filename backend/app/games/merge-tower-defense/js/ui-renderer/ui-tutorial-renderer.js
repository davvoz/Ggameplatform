...
export class TutorialRenderer {
    ...
    render(game, shopButtons) {
        ...
        if (currentStep.highlightShopButtons && shopButtons && shopButtons.length > 0) {
            this.drawDarkOverlay(shopButtons, ctx, game, width, height);
        } else {
        ...
        if (currentStep.highlightArea) {
            this.highlightInteractiveArea(currentStep, ctx, game);
        }

        // Highlight circle
        if (currentStep.highlightCircle) {
            this.renderHighlightCircle(currentStep, ctx, game);
        }

        // Highlight shop buttons
        if (currentStep.highlightShopButtons && shopButtons && game.state) {
            this.highlightShopButtonEffects(game, shopButtons, ctx);
        }

        // Arrow
        if (currentStep.arrow) {
            this.renderArrow(currentStep, game, ctx);
        }

        // Tutorial dialog
        if (game.tutorial.showDialog) {
            this.renderTutorialDialog(width, currentStep, ctx, height, game);
        }
    }

    renderTutorialDialog(width, currentStep, ctx, height, game) {
        ...
        for (const word of words) {
            currentLine = this.wrapTextLine(word, currentLine, wrappedLines, ctx, maxContentWidth);
        }
    ...
    wrapTextLine(word, currentLine, wrappedLines, ctx, maxContentWidth) {
        if (word.includes('\n')) {
            currentLine = this.splitAndWrapText(word, currentLine, wrappedLines);
        } else {
            currentLine = this.wrapTextWithWord(currentLine, word, ctx, maxContentWidth, wrappedLines);
        }
        return currentLine;
    }

    wrapTextWithWord(currentLine, word, ctx, maxContentWidth, wrappedLines) {
    ...
    splitAndWrapText(word, currentLine, wrappedLines) {
    ...
    highlightInteractiveArea(currentStep, ctx, game) {
    ...
    renderHighlightCircle(currentStep, ctx, game) {
    ...
    renderArrow(currentStep, game, ctx) {
    ...
    highlightShopButtonEffects(game, shopButtons, ctx) {
        ...
        shopButtons.forEach((button) => {
            ...
            if (game.state.cannonPriceMultiplier?.[cannonType]) {
    ...
    drawDarkOverlay(shopButtons, ctx, game, width, height) {
