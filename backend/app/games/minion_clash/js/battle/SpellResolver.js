...
export class SpellResolver {
    ...
    cast(card, x, y, team) {
        ...
        switch (sp.type) {
            case 'aoe_damage':       return this._aoeDamage(sp, x, y, team);
            case 'aoe_damage_slow':  return this._aoeDamageSlow(sp, x, y, team);
            case 'single_damage':    return this._singleDamage(sp, x, y, team);
    ...
    _aoeDamage(sp, x, y, team) {
        const enemies = this._filterTargets(
            this._spatial.queryByTeam(x, y, sp.radius, opposingTeam(team)), sp);
        for (const t of enemies) t.takeDamage(sp.damage, null, null);
    }

    _aoeDamageSlow(sp, x, y, team) {
        const enemies = this._filterTargets(
            this._spatial.queryByTeam(x, y, sp.radius, opposingTeam(team)), sp);
        for (const t of enemies) {
            t.takeDamage(sp.damage, null, null);
            if (!t.isDead()) {
    ...
    _singleDamage(sp, x, y, team) {
        ...
        best?.takeDamage(sp.damage, null, null);
    }
