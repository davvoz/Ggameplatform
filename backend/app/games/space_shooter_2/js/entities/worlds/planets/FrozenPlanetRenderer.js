...
export class FrozenPlanetRenderer extends PlanetRenderer {
    ...
    build(theme) {
        ...
        this.generateCrevasses(crevasseCount, W, H);
        ...
        const eH = fcfg?.edgeHue ? fcfg.edgeHue : [195, 220];
        const eL = fcfg?.edgeLit ? fcfg.edgeLit : [30, 45];
        const eS = fcfg?.edgeSat ? fcfg.edgeSat : [20, 40];
        const positions = this._distributeEdgeElements(count, W, H);
        this.generateEdgeGlaciers(count, positions, eR, eH, eS, eL);

        // Frozen lakes — large reflective ovals
        this.generateFrozenLakes(fcfg, W, H);

        // Snowfall layer
        this.generateSnowflakes(fcfg, W, H);

        this._vigRGB = fcfg ? fcfg.vigCol : '20,35,60';
    }

    generateSnowflakes(fcfg, W, H) {
        this._snowflakes = [];
        const snowfall = fcfg?.snowfall;
        if (snowfall) {
    ...
    generateEdgeGlaciers(count, positions, eR, eH, eS, eL) {
    ...
    generateCrevasses(crevasseCount, W, H) {
        for (let c = 0; c < crevasseCount; c++) {
            ...
            const totalCrevasseH = points.at(-1).y;
            //prefer .at. 
    ...
    update(dt) {
        ...
        this.scrollIceCrevasses(dt);
        // Scroll frozen lakes
        this.scrollFrozenLakes(dt);
        // Update snowflakes
        this.updateSnowflakePositions(dt);
    }

    updateSnowflakePositions(dt) {
    ...
    renderEdgeGlaciers(ctx) {
        if (this._edgeGlaciers) {
            for (const eg of this._edgeGlaciers) {
                ...
                // Soft shadow
                this.renderSoftShadow(ctx, eg, n, rx, ry, cx, cy);

                // Snow bank body — rounded with curves
                this.renderGlacierShape(ctx, eg, n, rx, ry, cx, cy);

                // Snow cap highlight
                this.renderSnowCapHighlight(ctx, eg, cx, rx, cy, ry);

                // Specular dot
                this.renderGlacierShine(eg, ctx, cx, rx, cy, ry);
            }
        }
    }

    renderGlacierShine(eg, ctx, cx, rx, cy, ry) {
        if (eg.hasShine) {
    ...
    renderSnowCapHighlight(ctx, eg, cx, rx, cy, ry) {
    ...
    renderGlacierShape(ctx, eg, n, rx, ry, cx, cy) {
    ...
    renderSoftShadow(ctx, eg, n, rx, ry, cx, cy) {
    ...
    _renderFrozenLakes(ctx) {
        ...
        for (const lk of this._frozenLakes) {
            ...
            for (const tileOff of [0, totalH]) {
                ...
                ctx.quadraticCurveTo(0, -lk.ry * 0.2, lk.rx * 0.35, lk.ry * 0);
                ctx.stroke();
                // Edge rim
