...
// ═══════════════════════════════════════════════════════
export function renderShipCard(ctx, options) {
    const { ship, x, y, alpha, game, w } = options;
    ctx.save();
...
//  Mini-Boss card
// ═══════════════════════════════════════════════════════
export function renderMiniBossCard(ctx, options) {
    const { mb, x, y, alpha, itemT, game, w } = options;
    ctx.save();
    ctx.globalAlpha = alpha;
    ...
    renderBossPartsAtPosition(ctx, { assets: game.assets, def: mb, centerX: x, centerY: y, scale: 1.1, spread, time: itemT });
...
//  Boss card
// ═══════════════════════════════════════════════════════
export function renderBossCard(ctx, options) {
    const { boss, x, y, alpha, scale, itemT, game, w,  index } = options;
    ctx.save();
    ctx.globalAlpha = alpha;
    ...
    renderBossPartsAtPosition(ctx, { assets: game.assets, def: boss, centerX: x, centerY: y, scale, spread, time: itemT });
...
// ═══════════════════════════════════════════════════════
export function renderW4EnemyCard(ctx, options) {
    const { enemy, x, y, alpha,  itemT, game, w } = options;
    ctx.save();
    ctx.globalAlpha = alpha;
    ...
    // ── Draw enemy — prefer pre-rendered spritesheet, fallback procedural ──
    ctx.globalAlpha = alpha;
    const sz = 90;
    const t = itemT;
    const frames = game?.assets?.getSprite(`enemy_${enemy.type}_frames`);
    if (frames && frames.length > 0) {
        const frameIdx = Math.floor((t * 6) % frames.length);
        const frame = frames[frameIdx];
        ctx.drawImage(frame, x - sz / 2, y - sz / 2, sz, sz);
    } else {
        drawW4Sprite(ctx, {
    ...
    const displayName = enemy.name || enemy.type.replaceAll('_', ' ').toUpperCase();
