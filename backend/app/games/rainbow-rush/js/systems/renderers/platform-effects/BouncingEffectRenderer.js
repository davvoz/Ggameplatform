...
export class BouncingEffectRenderer extends PlatformEffectRenderer {
    ...
    renderWaves(platform, x, y, time) {
        for (let i = 0; i < BouncingEffectRenderer.WAVE_COUNT; i++) {
            ...
            this.renderer.drawRect(x - 5, waveY, platform.width + 10, 2, [0.5, 0.3, 1, waveAlpha]);
        }
    }
    ...
    renderMotionLines(platform, x, y, time) {
        for (let i = 0; i < BouncingEffectRenderer.LINE_COUNT; i++) {
            ...
            this.renderer.drawRect(lineX, y + lineOffset, 2, platform.height - Math.abs(lineOffset * 2), [0.6, 0.4, 1, lineAlpha]);
        }
    }

    renderIdleState(platform, x, y, time) {
        const pulse = Math.sin(time * 3) * 0.3 + 0.5;
        this.renderer.drawRect(x, y, platform.width, 2, [0.5, 0.3, 1, pulse]);
        this.renderer.drawRect(x, y + platform.height - 2, platform.width, 2, [0.5, 0.3, 1, pulse]);
    }

    renderIndicators(platform, x, y, time) {
        const indicatorSize = 4;
        const indicatorPulse = Math.sin(time * 5) * 0.4 + 0.6;
        this.renderer.drawCircle(x + indicatorSize, y + platform.height / 2, indicatorSize, [0.7, 0.5, 1, indicatorPulse]);
        this.renderer.drawCircle(x + platform.width - indicatorSize, y + platform.height / 2, indicatorSize, [0.7, 0.5, 1, indicatorPulse]);
    }
}
