...
class AssetManager {
    ...
    createAdvancedExplosionFrame(frame) {
        ...
        // Seeded pseudo-random for deterministic but organic shapes
        ...
        // Draw multiple overlapping soft blobs to create organic fireball
        ctx.globalCompositeOperation = 'lighter';

        const blobCount = 5;
        for (let i = 0; i < blobCount; i++) {
            const angle = (i / blobCount) * TAU + rng(i) * 1.2;
            const dist = (8 + rng(i + 10) * 12) * expand;
            const bx = cx + Math.cos(angle) * dist;
            const by = cy + Math.sin(angle) * dist;
            const br = (14 + rng(i + 20) * 10) * expand;

            // Inner hot color
            const hue = 20 + rng(i + 30) * 30; // orange-yellow range
            const lightness = 65 + (1 - progress) * 25;
            const blobAlpha = baseAlpha * (0.5 + rng(i + 40) * 0.5);

            const grad = ctx.createRadialGradient(bx, by, 0, bx, by, br);
            grad.addColorStop(0, `hsla(${hue},100%,${lightness}%,${blobAlpha})`);
            grad.addColorStop(0.4, `hsla(${hue - 10},100%,${lightness - 20}%,${blobAlpha * 0.6})`);
            grad.addColorStop(1, `hsla(${hue - 15},100%,30%,0)`);
        ...
        // Central white-hot core (smaller, brighter)
        if (frame < 5) {
            const coreR = (10 + frame * 3) * expand;
            const coreA = baseAlpha * 0.9;
            const coreGrad = ctx.createRadialGradient(cx, cy, 0, cx, cy, coreR);
            coreGrad.addColorStop(0, `rgba(255,255,250,${coreA})`);
            coreGrad.addColorStop(0.3, `rgba(255,240,180,${coreA * 0.7})`);
            coreGrad.addColorStop(0.7, `rgba(255,150,50,${coreA * 0.3})`);
