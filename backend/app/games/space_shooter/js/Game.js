...
class Game {
    ...
    renderPauseButton(ctx) {
        ...
        // Background con bordo arrotondato
        ...
        // Border
        ctx.strokeStyle = isHover ? 'rgba(0, 220, 255, 0.8)' : 'rgba(0, 150, 255, 0.5)';
        ctx.lineWidth = 2;
        ctx.stroke();
        
        ctx.shadowBlur = 0;
        
        // Pause icon (due barrette)
        const iconPadding = 12;
        const barWidth = 5;
        const barHeight = h - iconPadding * 2;
        const gap = 6;
        const iconX = x + (w - barWidth * 2 - gap) / 2;
        ...
        // Left bar
        ctx.beginPath();
        ctx.roundRect(iconX, iconY, barWidth, barHeight, 2);
        ctx.fill();
        
        // Right bar
        ctx.beginPath();
        ctx.roundRect(iconX + barWidth + gap, iconY, barWidth, barHeight, 2);
        ctx.fill();
