...
class Enemy extends GameObject {
    ...
    update(deltaTime, game) {
        ...
        // ── Sprite idle animation tick ──
        this._animTimer += dt;
        if (this._animTimer >= 0.18) { // ~5.5 fps
            this._animTimer = 0;
            this._animIdx++;
        }
    ...
    renderEnemySprite(assets, ctx, cx, cy) {
        const pad = 8;
        // Animated sprite frames (if available)
        const frames = assets.getSprite(`enemy_${this.type}_frames`);
        if (frames) {
            const frame = frames[this._animIdx % frames.length];
            ctx.drawImage(frame, this.position.x - pad, this.position.y - pad, this.width + pad * 2, this.height + pad * 2);
            return;
        }
