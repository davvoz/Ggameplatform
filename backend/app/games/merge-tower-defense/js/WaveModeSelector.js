...
export class WaveModeSelector {
    ...
    render() {
        ...
        this.buttons.forEach((btn, index) => {
            const hoverOffset = Math.sin(this.animTime * 2 + index * 0.5) * 2;
            const y = btn.y + hoverOffset;

            // Button background
            const gradient = ctx.createLinearGradient(btn.x, y, btn.x + btn.width, y + btn.height);
            ...
            ctx.lineWidth = 6;
            Utils.drawRoundRect(ctx, btn.x + 3, y + 3, btn.width - 6, btn.height - 6, 10);
            ctx.stroke();

            // Mode label
            this.graphics.drawText(btn.mode.label.toUpperCase(), btn.x + btn.width / 2, y + 22, {
                size: 16,
            ...
            // Waves info
            this.graphics.drawText(`${btn.mode.waves} Waves`, btn.x + btn.width / 2, y + 45, {
                size: 22,
                color: '#ffffff',
                align: 'center',
                bold: true,
            ...
            // Reward
            this.graphics.drawText(`🏆 ${btn.mode.reward} Coins Reward`, btn.x + btn.width / 2, y + 68, {
                size: 14,
                color: '#ffdd00',
                align: 'center',
                shadow: true
