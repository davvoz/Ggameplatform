...
class DailyLoginBanner {
    constructor() {
        this.apiUrl = globalThis.ENV?.API_URL || globalThis.location.origin || 'http://localhost:8000';
    ...
    renderBanner(status) {
        ...
        const cycleLabel = totalCycles > 1 ? 's' : '';
            ...
            <div class="daily-login-content">
                ...
                ${totalCycles > 0 ? `
                    <div class="daily-login-cycles">
                        <span>🏆 Completed ${totalCycles} cycle${cycleLabel}!</span>
                    </div>
                ` : ''}
    ...
    async claimReward() {
        ...
        try {
            ...
            if (response.ok && data.success) {
                ...
                globalThis.dispatchEvent(new CustomEvent('dailyRewardClaimed', {
                    detail: data
                }));
            } else {
