...
function generateTabs() {
    ...
    html += `<button class="tab-btn" data-tab="er-diagram" onclick="switchTab('er-diagram')">🗺️ ER Diagram</button>`;
    container.innerHTML = html;
}
        ...
        html += `
            ...
            return `<th class="sortable" data-column="${col.key}" onclick="sortBy('${col.key}')">
                        ...
                        <div class="pagination" id="${key}Pagination" style="display: none;">
                            ...
                            <button class="pagination-btn" onclick="prevPage()" title="Pagina precedente">‹</button>
                            <span class="pagination-info" id="${key}PageInfo">Pagina 1 di 1</span>
                            <button class="pagination-btn" onclick="nextPage()" title="Pagina successiva">›</button>
                            <button class="pagination-btn" onclick="goToPage(AppState.totalPages)" title="Ultima pagina">»</button>
...
function updatePaginationButtons() {
    ...
    if (buttons[1]) buttons[1].disabled = isFirstPage; // ‹
    if (buttons[2]) buttons[2].disabled = isLastPage;  // ›
    if (buttons[3]) buttons[3].disabled = isLastPage;  // »
}
...
function updateCurrentSortInfo() {
    ...
    if (AppState.sortColumn) {
        ...
        const dirLabel = AppState.sortDirection === 'asc' ? '↑ Crescente' : '↓ Decrescente';
...
function updateActiveFiltersUI() {
        ...
        chipsContainer.innerHTML = AppState.filters.map((filter, index) => {
            ...
            return `
                        <span class="filter-chip">
                            ...
                            <button class="chip-remove" onclick="removeFilter(${index})">✕</button>
                        </span>
                    `;
        }).join('');
    } else {
    ...
    if (hasSort) {
        ...
        const fieldDef = schema?.fields[AppState.sortColumn];
        const fieldLabel = fieldDef?.label || AppState.sortColumn;
        const dirIcon = AppState.sortDirection === 'asc' ? '↑' : '↓';
        sortChip.innerHTML = `
                    <span class="sort-icon">↕️</span>
                    <span class="sort-text">${fieldLabel} ${dirIcon}</span>
                    <button class="chip-remove" onclick="clearSort()">✕</button>
                `;
...
async function showOpenSessions() {
    try {
        ...
        if (!data.success) {
            showToast('Errore nel caricamento delle sessioni aperte', 'error');
            return;
        }
        ...
        let html = `
                    <div class="modal-header" style="border-bottom-color: #e67700;">
        ...
        if (data.total_open === 0) {
            ...
            html += `
                        <div style="margin-bottom: 16px;">
                            <button onclick="closeAllOpenSessions()" 
                                    style="background: #dc3545; border: none; color: white; width: 100%; padding: 12px; border-radius: 6px; cursor: pointer; font-weight: bold; font-size: 14px;">
                            ...
                            <table style="width: 100%; border-collapse: collapse; font-size: 0.9em;">
                                <thead>
                                    <tr style="background: rgba(255,255,255,0.05); border-bottom: 2px solid rgba(255,255,255,0.1);">
                                        ...
                                        <th style="padding: 10px; text-align: left;">Inizio</th>
            ...
            data.sessions.forEach(session => {
                ...
                const durationColor = duration > 1800 ? '#ff6b6b' : '#51cf66';
                html += `
                            <tr style="border-bottom: 1px solid rgba(255,255,255,0.1);">
                                <td style="padding: 10px; font-family: monospace; font-size: 0.85em;">${session.session_id.substring(0, 12)}...</td>
                                <td style="padding: 10px;">${session.username || session.user_id.substring(0, 12)}</td>
                                <td style="padding: 10px;">${session.game_title || session.game_id}</td>
                                <td style="padding: 10px;">${Utils.formatDate(session.started_at)}</td>
                                <td style="padding: 10px; color: ${durationColor}; font-weight: bold;">${Utils.formatDuration(duration)}</td>
                                <td style="padding: 10px;">
                                    <button onclick="closeSingleSession('${session.session_id}')" 
                                            style="font-size: 0.85em; padding: 6px 12px; background: #ffc107; border: none; color: #000; cursor: pointer; border-radius: 4px; font-weight: 600;">
                                        🔒 Chiudi
                                    </button>
                                </td>
                            </tr>
        ...
        showToast('Errore nel caricamento delle sessioni aperte: ' + error.message, 'error');
    }
}
...
async function closeAllOpenSessions() {
    try {
        ...
        if (data.success) {
            closeCRUDModal();
            showToast(`${data.closed_count} sessioni chiuse con successo`, 'success');
            await checkOpenSessions();
            await refreshData();
        } else {
            showToast('Errore nella chiusura delle sessioni', 'error');
        }
    } catch (error) {
...
async function closeSingleSession(sessionId) {
    try {
        ...
        if (data.success) {
            showToast('Sessione chiusa con successo', 'success');
            await showOpenSessions(); // Ricarica il modal con le sessioni aggiornate
            await checkOpenSessions();
            await refreshData();
        } else {
            showToast('Errore nella chiusura della sessione', 'error');
        }
    } catch (error) {
