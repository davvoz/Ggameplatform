...
export class EffectRenderer {
    ...
    renderShield(player, time, x, y, radius) {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            this.renderer.drawCircle(x, y, auraRadius, [0.0, 0.95, 1, auraAlpha]);
        }
        ...
        for (let i = 0; i < sides; i++) {
            ...
            this.renderer.drawCircle(x1, y1, 7, [0.0, 0.9, 1, 0.4]); // Alone
            ...
            for (let s = 0; s <= steps; s++) {
                ...
                const px = x1 + (x2 - x1) * t;
                const py = y1 + (y2 - y1) * t;
                this.renderer.drawCircle(px, py, 3, [0.0, 0.9, 1, 0.8]);
            }
        }
        ...
        for (let i = 0; i < sides; i += 2) {
            ...
            if (electricPulse > 0.7) {
                ...
                this.renderer.drawCircle(sx, sy, 8, [0.0, 1, 1, electricPulse * 0.5]);
            }
        }

        // Centro LUMINOSO
        this.renderer.drawCircle(x, y, radius * 0.6, [0.0, 0.9, 1, 0.6]);
        this.renderer.drawCircle(x, y, radius * 0.4, [0.5, 1, 1, 0.9]);
        this.renderer.drawCircle(x, y, radius * 0.2, [1, 1, 1, 1.0]);
    }
    ...
    renderShieldBonus(player, time, x, y, avgRadius) {
        ...
        this.renderer.drawCircle(x, y, shieldRadius + 4, [0.0, 0.75, 1, shieldAlpha * 0.3]);
        this.renderer.drawCircle(x, y, shieldRadius, [0.0, 0.75, 1, shieldAlpha]);
    ...
    renderMagnetBonus(player, time, x, y) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            this.renderer.drawCircle(x, y, radius * (1 + pulseOffset), [1, 0.5, 0.0, alpha]);
        }
    ...
    renderTurboEffects(player, time, x, y, radius) {
        ...
        for (let i = 0; i < 8; i++) {
            ...
            this.renderer.drawCircle(ox, oy, 7, [1, 0.85, 0.0, 0.4]);
        }
        
        // Bordo dorato spesso
        this.renderer.drawCircle(x, y, radius + 8, [1, 0.9, 0.1, 0.8]);
        this.renderer.drawCircle(x, y, radius + 5, [1, 0.95, 0.3, 1.0]);
    ...
    renderBoostEffect(player, time, x, y, radius) {
        ...
        // Aloni cyan
        this.renderer.drawCircle(x, y, radius * 2.5, [0.0, 1, 0.9, boostPulse * 0.2]);
        this.renderer.drawCircle(x, y, radius * 1.8, [0.0, 0.8, 1, boostPulse * 0.18]);
        ...
        for (let i = 0; i < 4; i++) {
            ...
            const lineColor = [0.0, 1, 0.9, lineAlpha];

            this.renderer.drawRect(lineX - lineLength, y - 2, lineLength, 4, lineColor);
        }

        // Tint cyan
        this.renderer.drawCircle(x, y, radius * 1.15, [0.0, 1, 0.9, 0.2 * boostPulse]);
    }

    getActivePowerups(player) {
        const activePowerups = [];
        if (player.powerups.immortality) activePowerups.push({ type: 'immortality', color: [1, 0.84, 0.0, 1.0] });
        if (player.powerups.flight) activePowerups.push({ type: 'flight', color: [0.4, 0.85, 1, 1.0] });
        if (player.powerups.superJump) activePowerups.push({ type: 'superJump', color: [1, 0.2, 0.6, 1.0] });
